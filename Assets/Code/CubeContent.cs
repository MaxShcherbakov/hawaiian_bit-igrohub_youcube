...
public class CubeContent : MonoBehaviour
{
    ...
    IEnumerator HideCube(int row_x, int row_y, int row_z)
    {
        GameObject cube = GetOrCreateCube(row_x, row_y, row_z);
        while(cube.transform.localScale.x > 0)
        {
            Vector3 temp = cube.transform.localScale;
    ...
    IEnumerator ShowCube(int row_x, int row_y, int row_z)
    {
        GameObject cube = GetOrCreateCube(row_x, row_y, row_z);
        ...
        {
            Vector3 temp = cube.transform.localScale;
            temp.x = temp.y = temp.z = temp.x * 1.05f;
            cube.transform.localScale = temp;
            yield return null;
        }
    }
    ...
    public void AddColored(int count, Color new_color)
    {
        ...
        while (GetCubesCount(World.colors) < new_colored_count)
        {
            ...
            StartCoroutine(ShowCube(row_x, row_y, row_z));
        }
    }
    ...
    public void RemoveColored(int count)
    {
        ...
        while (GetCubesCount(World.colors) > new_colored_count)
        {
            ...
            StartCoroutine( HideCube(row_x, row_y, row_z) );
        }
    }
}
